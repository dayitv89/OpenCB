// LT-PGN-VIEWER 3.492 (c) Lutz Tautenhahn (2001-2011)
...
var forceMove = false; // устанавливается для обработки окончания бранча
var isMoveSet = false;
...
{
    var w_sqs = document.getElementsByClassName('WhiteSquares');

    for (var i = 0; i < w_sqs.length; i++)
    {
        var sq = w_sqs[i];
        var b = sq.getAttribute('background');
        var tokens = b.split('/');
        sq.setAttribute('background', style + tokens[1] + '/' + tokens[2]);
    }

    var b_sqs = document.getElementsByClassName('BlackSquares');
    
    for (var i = 0; i < b_sqs.length; i++)
    {
        var sq = b_sqs[i];
        var b = sq.getAttribute('background');
        var tokens = b.split('/');
...
//kk:to support UIWebView, redefine call from js:
//  if (TargetDocument) HighlightMove("m"+MoveCount+"v"+CurVar);
//kk:to href:
location.href = '/HighlightMove?' + "m" + MoveCount + "v" + CurVar;

/*kk:disabling unknown calls:
...
function MoveForward(nn, rr) {
    MoveForward(nn, rr, false);
}

function MoveForward(nn, rr, force) { 
    var ii, ffst = 0, llst, ssearch, ssub, ffull, mmove0 = "", mmove1 = "";
    var href2, entryMove = MoveCount, routes = new Array(), vars = new Array();
    ...
    if (mmove1 != "") {
        //mmove1 - следующий ход после выбранного 
        //CurVar - текущая ветка ходов
        //dl: формирование сcылок для выбора хода при наличие вариантов:
        //MoveType - 0 - ход белых, 1 - ход черных
        //StartMove - чей был первоначальный ход: 0 - белых, 1 - черных
        //MoveCount -  номер хода куда переходим (при первом ходе/выборе хода в книге идет перерасчет на правильный номер хода выше по коду);
        //ShortPgnMoveText[0] - массив веток ходов
        //ShortPgnMoveText[1] - из какой ветки вызывается текущая ветка с индексом массива
        //ShortPgnMoveText[2] - с какого хода начинается текущая ветка с индексом массива
        //HistMove[(MoveCount-StartMove)-1] - история ходов - номер хода куда переходим c учетом кто начинал партию (???).
        if(!isCalculating &&
           !forceMove && /* НЕ возврат из бранча по его окончанию */
           !force && !isMoveSet) {/* НЕ рекурсивный вызов из SetMove, и НЕ ответ на запрос перехода в вариант*/
            for (var i = 0; i < ShortPgnMoveText[2].length; i++) { /* в этом массиве номера ходов, с которых начинаются соответствующие варианты */  
                // собираем номера вариантов, доступных для данной позиции 
                // второе условие - выбираем варианты, доступные только из текущей ветки
                if ((MoveCount - 1) == ShortPgnMoveText[2][i] && CurVar == ShortPgnMoveText[1][i]) { 
                    routes.push(i);
                    vars.push(Uncomment(ShortPgnMoveText[0][i]));              
                }
            }
            //%23%23 = ## , %E5%E5 = ^^
            if (routes.length > 0 && routes[0] > CurVar && ShortPgnMoveText[1][routes[0]] == CurVar) {  
                // выводим следущий ход ТЕКУЩЕЙ ветки, чтобы не выкидывал всегда на основную ветку
                href2 = '/ConfirmBranchSelection??SetMove(' + MoveCount + ',' + CurVar + ')' + '%5E%5E' + HistMove[(MoveCount-StartMove)-1];
                for (var j=0; j < routes.length; j++) {
                    var nextMoveCount = (MoveCount - 1) + 1;
                    href2 = href2 + '%23%23SetMove(' + nextMoveCount + ',' + routes[j] + ')' + '%5E%5E' + vars[j];
                }
                location.href = href2;
                return;
            }                
    ...
    /* ход невозможен - нужен выход из варианта и продолжение основного маршрута */
    if (entryMove == MoveCount) {
        if(CurVar > 0) {
            forceMove = true;
            SetMove(ShortPgnMoveText[2][CurVar]+2, ShortPgnMoveText[1][CurVar]);
            return;
        }
    }
    ...
    if (document.BoardForm) { 
        if ((document.BoardForm.Position)&&(mmove1 != ""))
            document.BoardForm.Position.value=TransformSAN(HistMove[MoveCount-StartMove-1]);
        if ((mmove1 != "") && (isDragDrop) && (nn == 1) && (!dragObj) && (dragPiece[0] >= 0) && (!rr) && (!isAnimating)) 
            AnimateBoard(1);
        else 
            RefreshBoard();
    }
...
{ var ii, ffrom="", ccapt=0, ll, yy1i=-1;
  ...
  { if ((mm.indexOf("O")>=0)||(mm.indexOf("0")>=0))
    { if ((mm.indexOf("O-O-O")>=0)||(mm.indexOf("0-0-0")>=0)||(mm.indexOf("OñOñO")>=0)||(mm.indexOf("0ñ0ñ0")>=0)) 
      { if (EvalMove(ttype0, 6, xx0, yy0, ttype1, xx1, yy1, ccapt, sstore))
          return(1);
        return(0);
      }
      if ((mm.indexOf("O-O")>=0)||(mm.indexOf("0-0")>=0)||(mm.indexOf("OñO")>=0)||(mm.indexOf("0ñ0")>=0))
    ...
    if ((mm.indexOf("---")>=0)||(mm.indexOf("ñññ")>=0))
  ...
  { for (ii=0; ii<5; ii++)
    ...
    { if ((ffrom.charAt(ll-1)=="-")||(ffrom.charAt(ll-1)=="ñ")) ll--; //Smith Notation
    }
...
function SetBoardClicked(nn) { 
    ...
    if (CandidateStyle != "") 
        HighlightCandidates(nn,CandidateStyle);
    if (isDragDrop) { 
        BoardClicked = nn; 
        return; 
    }
    ...
    if (BoardClicked >= 0) { 
        if (BoardClicked < 64) { 
            ...
            document.images[ImageOffset+BoardClicked+3].style.borderColor = "#FF0000";
    }
}
...
function BoardClick(nn, bb, force)
    ...
    { mm="---";
      ...
      if (force == false) {
          if (MoveCount%2 == 0) { 
              ...
              window.location = 'cocoa:confirm_white_nullmove:' + nn;
          }
          else { 
              //if (!confirm("Black nullmove?")) return; 
              window.location = 'cocoa:confirm_black_nullmove:' + nn;              
          }
          return;
      }
...
function ShowCapturedPieces(bb) { 
    ...
    if (isCapturedPieces) 
        ...
        if (document.images["RightLabels"]) 
            kk0++;
        for (kk=0; kk<32; kk++) 
            SetImg(64+kk0+kk,LabelPic[4]);
        if ((parent) && (parent.ChangeColWidth)) 
            parent.ChangeColWidth(0);
    }
}
...
/*kk: to enable WebView processing changing href text:
        if (! nnoscript) ss+="<a href=\"javascript:SetMove{{"+MoveCount+","+vv+gg+"}}\" name=\"m"+MoveCount+"v"+vv+"\">";
*/
        if (! nnoscript) ss+="<a href=\"/SetMove{{"+MoveCount+","+vv+gg+"}}\" name=\"m"+MoveCount+"v"+vv+"\">";
