...
void Position::do_move(Move m, UndoInfo &u, Bitboard dcCandidates)
{
    ...
    else
    {
        ...
        if (piece == PAWN)
        {
            ...
            {
                if ((us == WHITE && (this->white_pawn_attacks(from + DELTA_N) & this->pawns(BLACK))) ||
                    (us == BLACK && (this->black_pawn_attacks(from + DELTA_S) & this->pawns(WHITE))))
                {
...
Key Position::compute_key() const
{
    ...
    for (Square s = SQ_A1; s <= SQ_H8; s++)
    {
        ...
        {
            result ^= zobrist[this->color_of_piece_on(s)][this->type_of_piece_on(s)][s];
        }
    }
...
{
    // Draw by material?
    if (!this->pawns() && this->non_pawn_material(WHITE) + this->non_pawn_material(BLACK) <= BishopValueMidgame)
    {
        return true;
    }
