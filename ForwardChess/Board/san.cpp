...
std::vector<std::string> line_to_moves(const Chess::Position &pos, Chess::Move line[], int startColumn, bool breakLines, int moveNumbers)
{
    std::vector<std::string> moves;
    
    Chess::Position p = Chess::Position(pos);
    Chess::UndoInfo u;
    std::string moveStr;
    ...
    for (int i = 0; line[i] != Chess::MOVE_NONE; i++)
    {
        moves.push_back(move_to_san(p, line[i]));
...
Move move_from_san(Position &pos, const std::string &movestr) {
  ...
  else if(movestr == "O-O" || movestr == "0-0") {
    Move m;
    while((m = mp.get_next_move()) != MOVE_NONE)
  ...
  *cc = '\0';
  ...
  int left = 0, right = static_cast<int>(strlen(str)) - 1;
...
const std::string line_to_san(const Position &pos, Move line[], int startColumn, bool breakLines, int moveNumbers)
{
  ...
  for(int i = 0; line[i] != MOVE_NONE; i++) {
    ...
    if(breakLines && length > maxLength) {
      ...
      for(int j = 0; j < startColumn; j++)
        s << " ";
      length = static_cast<int>(moveStr.length()) + 1;
    }
    s << ns.str() << moveStr << " ";
