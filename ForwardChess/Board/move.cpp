...
Move fix_castling_and_promotion(const Position &pos, Move m)
{
    Move mlist[32];
    int j, n;
    n = pos.moves_from(move_from(m), mlist);

    for (j = 0; j < n; j++)
    {
        if (move_to(mlist[j]) == move_to(m) && move_promotion(mlist[j]) == move_promotion(m))
        {
            return mlist[j];
        }
        else if (move_is_short_castle(mlist[j]) && pos.type_of_piece_on(move_from(m)) == KING && move_to(m) - move_from(m) == 2)
        {
            return mlist[j];
        }
        else if(move_is_long_castle(mlist[j]) && pos.type_of_piece_on(move_from(m)) == KING && move_to(m) - move_from(m) == -2)
        {
            return mlist[j];
        }
    }
