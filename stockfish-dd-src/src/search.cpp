...
extern void __analysisHasReceived__(const std::vector<std::string> &lines);
...
*/
  ...
  string uci_pv(const Position& pos, int depth, Value alpha, Value beta) {
    ...
    for (size_t i = 0; i < uciPVSize; ++i)
    {
        ...
        const std::string score = (i == PVIdx ? score_to_uci(v, alpha, beta) : score_to_uci(v));
        ...
        std::stringstream ss;
        ss << (i + 1) << ". (" << v << ") depth=" << depth << " ";
        for (size_t j = 0; RootMoves[i].pv[j] != MOVE_NONE; ++j)
            ss <<  " " << move_to_uci(RootMoves[i].pv[j], pos.is_chess960());
    
        engineSendAnalysis(i, pos, RootMoves, uciPVSize, d, v);
    }
